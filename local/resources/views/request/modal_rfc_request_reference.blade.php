    ...
    <h5>Reference Number (RFC):</h5>
      <select id="rfc_request_reference">
        ...
        @foreach($granted_request_types as $request_type)
          <option value="{{$request_type->req_code}}.{{$request_type->project_no}}.{{$request_type->req_desc}}">{{$request_type->req_code}} - {{$request_type->req_desc}} - {{$request_type->project_no}}</option>
        @endforeach
      </select>
